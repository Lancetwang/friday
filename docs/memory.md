...
Friday deterministically detects explicit memory signals such as "remember", "from now on", preferences, and corrections. It saves the original user statement to the current dated Markdown file with hidden source and session metadata. Short, high-confidence identity and communication preferences are also promoted to `USER.md`; project-like or ambiguous future instructions remain episodic evidence. Credential-like content is rejected.
...
Entries are ordinary Markdown bullets. Hidden HTML comments carry ids, sources, and timestamps; Friday removes those comments from the model prefix.
...
`/compact` first gives Friday a chance to persist durable facts through `friday memory`, then rebuilds the live context from the fresh prefix, structured summary, latest ten complete user turns, and one current progress checkpoint. Compact summaries and temporary task progress are never stored as long-term memory.
