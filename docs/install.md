...
```bash
npm install --global ./friday-agent-core-0.9.0.tgz ./friday-agent-0.9.0.tgz
friday --version
```
...
The same GitHub Release also contains `friday-agent-core-0.9.0.tgz` as a
fallback before registry publication:

```bash
npm install ./friday-agent-core-0.9.0.tgz
```
