...
A run has no step limit and no spend ceiling. It ends when the work is done, when it stops making progress, or when the context window can no longer be made to fit. Neither the number of requests nor their cumulative token usage may end a run: because every step re-sends the conversation, cumulative usage grows with the square of the step count and crosses any fixed ceiling while the window is still mostly empty. Both figures are recorded for the trace and the cost shown in the UIs, and compared against nothing.

The window is therefore the only resource bound, and compaction is what keeps it from being reached. At 85% occupancy the guard reclaims tool results losslessly, then rewrites the conversation in place — a summary plus the most recent tool cycles — and the run continues with the same agent. Only a window that neither pass can bring back under 85% gives the active Agent one final answer without tools. The answer, verification evidence, and stop reason remain in the trace.
